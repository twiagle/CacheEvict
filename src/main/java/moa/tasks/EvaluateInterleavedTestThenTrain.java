...
public class EvaluateInterleavedTestThenTrain extends ClassificationMainTask {
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
       ...
       // this.streamOption.setValueViaCLIString(streamString + " -i " + this.randomSeedOption.getValueAsCLIString());
        ...
        double RAMHours = 0.0;
        String outFileName = DoTask.outFileName;
        ...
        try (BufferedWriter out = new BufferedWriter(new FileWriter(outFileName))){
            while (stream.hasMoreInstances()
                ...
                if (instancesProcessed > Config.ORIGINAL.getExcludeLength() ) {//cdn10w,wiki
//                if (instancesProcessed > 900000) {//zipf
                    StringBuilder outLine = new StringBuilder();
                    if (prediction.length == 0)  {
                        DoTask.driftNum++;
                        outLine.append(instancesProcessed).append(' ').append(1)
                                .append(' ').append(10);
                        outLine.append("\n");
                        out.write(outLine.toString());
                    }else{

                        outLine.append(instancesProcessed).append(' ').append(Utils.maxIndex(prediction))
                                .append(' ').append(prediction[Utils.maxIndex(prediction)]);
                        outLine.append("\n");
                        out.write(outLine.toString());
                    }
                }
