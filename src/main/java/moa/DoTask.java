...
public class DoTask {
    ...
    public static int driftNum=0;
    public static String tbDoTask(String featureFile) {

        //CACHESIZE = cacheSize;
        StringBuilder outName = new StringBuilder();
        outName.append(featureFile.substring(0,featureFile.lastIndexOf('.')));
        //outName.append("./data/50w-cache");
        //outName.append(CACHESIZE);
        outName.append(".decision");
        outFileName=outName.toString();

        //  -c 9 for 1,2,3,4,5,6,7,8
//        String [] args = {"EvaluateInterleavedTestThenTrain","-l","(meta.LearnNSE -p 20000 -e 50 -s AGE)","-s","(ArffFileStream -f ",featureFile,"-c 9) -i 10090000 -f 20000"};
//        String [] args = {"EvaluateInterleavedTestThenTrain","-l","(meta.LearnNSE -p 100000 -e 20 -s AGE)","-s","(ArffFileStream -f ",featureFile,"-c 9) -i 11000000 -f 100000"};
        String [] args = {"moa.tasks.EvaluateInterleavedChunks -l (moa.classifiers.meta.LearnNSE -p 100000 -e 20 -s AGE) -s (ArffFileStream -f ",featureFile,"-c -1) -i "+Config.ORIGINAL.getTotalLength()+" -f "+Config.ORIGINAL.getInternal()};
//        String [] args = {"moa.tasks.EvaluateInterleavedChunks -l (meta.LearnNSE -p 100000 -e 20 -s AGE) -c 100000 -s (ArffFileStream -f ",featureFile,"-c -1) -i "+Config.ORIGINAL.getTotalLength()+" -f "+Config.ORIGINAL.getInternal()};
//        String [] args = {"EvaluateInterleavedTestThenTrain","-l","(meta.LearnNSE -p 100000 -e 20 -s AGE)","-s","(ArffFileStream -f ",featureFile,"-c -1) -i "+Config.ORIGINAL.getTotalLength()+" -f "+Config.ORIGINAL.getInternal()+""};
        //5320000EvaluateInterleavedChunks
        try {
            if (args.length < 1) {
                System.err.println();
                System.err.println(Globals.getWorkbenchInfoString());
        ...
        System.out.println("driftNum:"+driftNum);
        return outFileName;
    }
}
